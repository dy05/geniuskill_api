                ...
                <form action="{{ route('quizzes.update', $quiz) }}" method="POST">
                    ...
                    <div class="flex flex-col gap-4 mb-2 mt-4">
                        ...
                        <div class="flex flex-col md:flex-row-reverse gap-4 justify-between">
                            ...
                            <div class="flex flex-col gap-3" id="choices">
                                @forelse($quiz->choices as $choice)
                                    @php
                                        $id = (int)$choice->id ?? 0;
                                        if ($id > $choicesId) {
                                            $choicesId = $id;
                                    ...
                                    <div class="flex items-center gap-2" id="choices-{{ $id }}">
                                        ...
                                        <input type="radio" class="rounded" id="response" name="response"
                                               @checked((string)$quiz->response === (string)$id) value="{{ $id }}" required/>
                                    </div>
                                @empty
                                    @php
                                        $id = 0;
                    ...
                    <div class="flex flex-col gap-4 mb-2 mt-4">
                        ...
                        <select name="course_id" id="course_id" required>
                            <option value="" disabled @selected(!$quiz->course_id)>
                                Sélectionner un cours
                            </option>
                            @foreach($courses as $item)
                                <option value="{{ $item->id }}" @selected($quiz->course_id === $item->id)>
                                    {{ $item->label }}
                                </option>
                            @endforeach
                        </select>
